...
public OnPlayerCommandText(playerid, cmdtext[])
{
    ...
    if(strcmp(cmdtext, "/pickgun", true) == 0
...
{
		if(GetPlayerState(playerid) != PLAYER_STATE_ONFOOT) return 1;
        new GunIDD = GetPlayerWeapon(playerid);
        new GunAmmoO = GetPlayerAmmo(playerid);
        if(GunIDD > 0 && GunAmmoO != 0)
        {
            new f = MAX_OBJ+1;
            for(new a = 0; a < MAX_OBJ; a++)
            {
            ...
            if(f > MAX_OBJ) return SendClientMessage(playerid, 0x33AA3300, "You Get Bug So You Dont Drop Your Gun After Death");
			RemovePlayerWeapon(playerid, GunIDD);
			dGunData[f][ObjData][0] = GunIDD;
			dGunData[f][ObjData][1] = GunAmmoO;
            GetPlayerPos(playerid, dGunData[f][ObjPos][0], dGunData[f][ObjPos][1], dGunData[f][ObjPos][2]);
            dGunData[f][ObjID] = CreateObject(GunObjects[GunIDD], dGunData[f][ObjPos][0], dGunData[f][ObjPos][1], dGunData[f][ObjPos][2]-1, 93.7, 120.0, 120.0);
            new buffer[50];
			format(buffer, sizeof(buffer), "You threw %s", GunNames[dGunData[f][ObjData][0]]);
			SendClientMessage(playerid, 0x33AA3300, buffer);
        }
